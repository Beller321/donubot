...
@cilik_cmd(pattern="promote(?:\s|$)([\s\S]*)")
...
async def spider(spdr):
    ...
    if mute(spdr.chat_id, user.id) is False:
        return await edit_delete(cilik, "**ERROR:** `Pengguna Sudah Dibisukan.`")
    try:
...
async def ungmoot(un_gmute):
    ...
    if not user:
        return
    await cilik.edit("`Membuka Global Mute Pengguna...`")
