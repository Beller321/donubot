...
@bot.on(poci_cmd(outgoing=True, pattern=r"anichar ?(.*)"))
...
@bot.on(poci_cmd(outgoing=True, pattern=r"airing ?(.*)"))
async def anilist(event):
    search = event.pattern_match.group(1)
...
@bot.on(poci_cmd(outgoing=True, pattern=r"animanga ?(.*)"))
async def anilist(event):
    search = event.pattern_match.group(1)
    reply_to_id = event.message.id
    if event.reply_to_msg_id:
...
@bot.on(poci_cmd(outgoing=True, pattern=r"anilist ?(.*)"))
async def anilist(event):
