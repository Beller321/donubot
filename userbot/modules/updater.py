...
async def deploy(xx, repo, ups_rem, ac_br, txt):
    if HEROKU_API_KEY is not None:
        ...
        await edit_or_reply(
            xx, "`Cilik-Userbot Berhasil Di Deploy! Userbot bisa di gunakan kembali.`"
        )
...
async def update(xx, repo, ups_rem, ac_br):
    try:
        ...
        repo.git.reset("--hard", "FETCH_HEAD")
    await edit_or_reply(
        xx, "`Cilik-Userbot Berhasil Diupdate! Userbot bisa di Gunakan Lagi.`"
    )
...
async def upstream(event):
    ...
    off_repo = b64decode(
        "aHR0cHM6Ly9naXRodWIuY29tL2dyZXk0MjMvQ2lsaWtVc2VyYm90"
    ).decode("utf-8")
    force_update = False
    try:
        txt = (
    ...
    if conf == "deploy":
        await xx.edit("`[HEROKU]: Update Deploy Cilik-Userbot Sedang Dalam Proses...`")
        await deploy(xx, repo, ups_rem, ac_br, txt)
        return

    if changelog == "" and not force_update:
        await edit_delete(xx, "**✥ Cilik-Userbot Sudah Versi Terbaru ✥**")
        return repo.__del__()
